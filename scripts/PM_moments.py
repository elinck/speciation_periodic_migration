...
# import data
dd = Misc.make_data_dict_vcf(vcf_filename="vostok_sim_10388971.trees.vcf",popinfo_filename="id_pop_moments.txt")
pop_ids = ["1","2"]
projections = [20,20]
...
def PM(params, ns):
    ...
    fs.integrate([nu1, nu2], T3, m = np.array([[0, m], [m, 0]]))
    fs.integrate([nu1, nu2], T4, m = np.array([[0, 0], [0, 0]]))
    fs.integrate([nu1, nu2], T5, m = np.array([[0, m], [m, 0]]))
    fs.integrate([nu1, nu2], T6, m = np.array([[0, 0], [0, 0]]))
    fs.integrate([nu1, nu2], T7, m = np.array([[0, m], [m, 0]]))
    return  fs

func=PM
upper_bound = [200,200,200,200,200,200,200,200,200,200,200]
...
for i in range(25):
        ...
        fst=data.Fst()
        PMmod=open('PM_params.txt','a')
        PMmod.write(str(poptg[0])+'\t'+
            str(poptg[1])+'\t'+
            str(poptg[2])+'\t'+
            str(poptg[3])+'\t'+
            str(poptg[4])+'\t'+
